...
```html
	<!--工作目录 暂存区 本地仓库 远程仓库-->
	* git add 路径      添加指定文件到暂存区。      git add ./ . * —A  添加所有修改过的文件到暂存区
	* git commit -m "备注一些修改信息" 。                   将暂存区的文件放到版本库中
	* git commit --all -m "备注一些修改信息"          一次性将所有修改过的文件提交到版本库
	* git status查看文件状态命令。绿色为放到了暂存区，红色表示文件已修改但是还未添加到暂存区。dir clear 表示文件被放到了仓库
	* git log 查看提交日志                       git log --oneline  查看简洁版的日志
	* git ignore 忽略文件，被忽略的文件不会被添加到仓储库中   必须以/开头  一行写一个路径

		* / .idea  会忽略.idea文件
		* /js   会忽略js目录里的所有文件
		* /js/*.js  会忽略js目录下所有的js文件
	* git reset --hard Head~0  回退到指定的版本   0 为索引    Head永远指向指向最新的版本，查看日志只会打印最新版本和之前的版本

        
	* git reset --hard 版本号（随机的字符串）                     可以通过版本号精确的会退到某一次提交的的版本号    版本号可以通过日志来获取
	* git reflog  可以查看每一次切换版本的记录
	* git branch dev（分支名）  创建分支，在刚创建时，分支的内容和master分支里面的东西一样
	* git checkout dev（分支名）  切换分支。
	* git branch 可以查看当前有哪些分支 
	* git merge dev   合并分支内容，把当前分支与指定的分支进行和并。   当前分支指的是git branch 命令输出的前面带有*的分支。  合并时如果有冲突需要手动解决冲突，然后在提交一次
	* git branch  -d dev       删除分支
	* git push [地址] master        当前分支上传到远程master分支
	* git pull  [地址] master         可以获取到远程分支的数据 （前提是要初始化一个本仓储）   多次使用会合并处理
	* git  clone [地址]           获取数据，多次使用会覆盖
	* ssh-keygen -t rsa -C "xiaoming@qq.com"    生成秘钥

        在本地修改文件上传之前先从服务器pull最新的版本，解决可能存在的冲突，然后在push最新的版本到服务器    
        git remote add origin 地址                       相当于定义了一个origin变量，保存了地址
        git push origin -u master                         -u 参数相当于将远程仓库和master进行了关联
...
git 命令:
1.创建文件
  echo "new Fun">> index.html

2.连接远程
git config __global user.name"lxn"
git config __global user.name"904441248@qq.com"

3.初始化 --创建一个git仓库
git init

4.将文件添加到临时位置
git add test.txt  // 使用 git add * 是将整个文件夹添加
...
6.添加仓库地址
git remote add origin git@github.com:tenbukilloneperson/Funtoucn.git

7.添加
git push -u origin master

//强制添加 git push -u origin +master
...
2.克隆一个项目
git clone url git@github.com:schacon/simplegit.git   ( git clone url )
